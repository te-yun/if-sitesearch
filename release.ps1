...
docker run -d --name ${DOCKER_IMAGE_NAME} -p 8001:8001 --env SECURITY_USER_PASSWORD=$env:SECURITY_USER_PASSWORD --env BUILD_NUMBER=$env:BUILD_NUMBER --env SCM_HASH=$env:SCM_HASH  -v ~/srv/${DOCKER_IMAGE_NAME}:/home/svc_usr/data intrafind/${DOCKER_IMAGE_NAME}:${DOCKER_TAG}
...
docker run -d --name $httpsRedirect -p 80:80 -p 443:443 intrafind/${httpsRedirect}:latest
